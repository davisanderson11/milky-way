...
public class ChunkVisualizer
{
    ...
    /// </summary>
    public void VisualizeChunk(string chunkId, string? outputPrefix = null)
    {
        ...
        GenerateTopView(positions, centerX, centerY, centerZ, $"{outputPrefix}_top.png");
        GenerateSideView(positions, centerX, centerY, centerZ, $"{outputPrefix}_side.png");
        GenerateFrontView(positions, centerX, centerY, centerZ, $"{outputPrefix}_front.png");
        
        // Generate composite view
        GenerateCompositeView(positions, centerX, centerY, centerZ, chunk, $"{outputPrefix}_composite.png");
        
        Console.WriteLine($"✓ Visualization complete!");
    }
    
    private void GenerateTopView(List<StarPosition> stars, double centerX, double centerY, double centerZ, string filename)
    {
        ...
        using (var surface = SKSurface.Create(new SKImageInfo(imageSize, imageSize)))
        {
            ...
            DrawStars(canvas, stars, s => s.X, s => s.Y, scale, offsetX, offsetY);
    ...
    private void GenerateSideView(List<StarPosition> stars, double centerX, double centerY, double centerZ, string filename)
    {
        ...
        using (var surface = SKSurface.Create(new SKImageInfo(imageSize, imageSize)))
        {
            ...
            DrawStars(canvas, stars, s => s.X, s => s.Z, scale, offsetX, offsetZ);
    ...
    private void GenerateFrontView(List<StarPosition> stars, double centerX, double centerY, double centerZ, string filename)
    {
        ...
        using (var surface = SKSurface.Create(new SKImageInfo(imageSize, imageSize)))
        {
            ...
            DrawStars(canvas, stars, s => s.Y, s => s.Z, scale, offsetY, offsetZ);
    ...
    private void GenerateCompositeView(List<StarPosition> stars, double centerX, double centerY, double centerZ, 
        ChunkBasedGalaxySystem.ChunkCoordinate chunk, string filename)
    {
        ...
        using (var surface = SKSurface.Create(new SKImageInfo(imageSize * 2, imageSize * 2)))
        {
            ...
            DrawSubView(canvas, stars, s => s.X, s => s.Y, 0, 0, subSize, "Top (X-Y)");
            canvas.Restore();
            ...
            DrawSubView(canvas, stars, s => s.X, s => s.Z, 0, 0, subSize, "Side (X-Z)");
            canvas.Restore();
            ...
            DrawSubView(canvas, stars, s => s.Y, s => s.Z, 0, 0, subSize, "Front (Y-Z)");
            canvas.Restore();
        ...
        double scale, double offsetX, double offsetY)
    {
        using (var paint = new SKPaint())
        {
            ...
            foreach (var star in stars)
            {
                ...
                var size = Math.Max(0.5f, Math.Min(5f, (float)Math.Sqrt(star.Star.Luminosity)));
                
                // Color from star
                var starColor = star.Star.Color;
                var brightness = Math.Min(1, star.Star.Luminosity * 0.3);
                var alpha = (byte)(brightness * 255);
                ...
                if (star.Star.Luminosity > 10)
                {
                    paint.Color = paint.Color.WithAlpha((byte)(alpha / 4));
    ...
    private void DrawSubView(SKCanvas canvas, List<StarPosition> stars,
        Func<StarPosition, double> getX, Func<StarPosition, double> getY,
        float offsetX, float offsetY, int size, string label)
    {
        // Calculate bounds
        ...
        using (var paint = new SKPaint())
        {
            ...
            foreach (var star in stars)
            {
                ...
                var starSize = Math.Max(0.5f, Math.Min(4f, (float)Math.Sqrt(star.Star.Luminosity) * 0.8f));
