...
public class SkyGenerator
{
    ...
    public void GenerateSkyView(double obsX, double obsY, double obsZ, int imageSize)
    {
        using (var surface = SKSurface.Create(new SKImageInfo(imageSize, imageSize)))
        {
            ...
            // Apply dust effect (dims areas outside spiral arms/bulge/core)
            ApplyDustEffect(canvas, imageSize, obsX, obsY, obsZ,
                dirX, dirY, dirZ, rightX, rightY, rightZ, upX, upY, upZ);
            
            // Apply shine effect (makes bright areas bleed)
            ApplyShineEffect(surface, imageSize);
        ...
        double upX, double upY, double upZ)
    {
        ...
        int numRays = 500000; // Increased ray count for better quality
    ...
    /// <summary>
    /// Apply dust effect that dims areas outside spiral arms, bulge, and core
    /// </summary>
    private void ApplyDustEffect(SKCanvas canvas, int imageSize, 
        double obsX, double obsY, double obsZ,
        double dirX, double dirY, double dirZ,
        double rightX, double rightY, double rightZ,
        double upX, double upY, double upZ)
    {
        ...
        {
            paint.IsAntialias = true;
            paint.BlendMode = SKBlendMode.Multiply; // Darken blend mode
            
            // Create a dust map based on galactic structure
            int dustResolution = 100; // Lower resolution for dust effect
            float cellSize = (float)(imageSize / (double)dustResolution);
            
            for (int dy = 0; dy < dustResolution; dy++)
            {
                for (int dx = 0; dx < dustResolution; dx++)
                {
                    // Convert screen coordinates to ray direction
                    double u = (dx + 0.5 - dustResolution/2.0) / (dustResolution/2.0) * scale;
                    double v = (dy + 0.5 - dustResolution/2.0) / (dustResolution/2.0) * scale;
                    ...
                    // Sample dust density along the ray
                    double totalDustOpacity = 0;
                    double stepSize = 500; // Large steps for dust
                    double maxDustDistance = 50000;
                    
                    for (double dist = 100; dist < maxDustDistance; dist += stepSize)
                    {
                        double px = obsX + rayX * dist;
                        double py = obsY + rayY * dist;
                        double pz = obsZ + rayZ * dist;
                        ...
                        // Check if we're in a low-density region (inter-arm)
                        var armMultiplier = GalaxyGenerator.CalculateSpiralArmMultiplier(pos);
                        var totalDensity = GalaxyGenerator.CalculateTotalDensity(pos);
                        
                        // Dust is present in inter-arm regions and outer disk
                        double dustDensity = 0;
                        
                        if (r > 3000 && r < 45000) // Within disk
                        {
                            // More dust in inter-arm regions
                            if (armMultiplier < 1.5f) // Inter-arm region
                            {
                                dustDensity = 0.8 * (1.5f - armMultiplier) / 0.5f;
                            }
                            
                            // Add general disk dust that decreases with height
                            double zScale = Math.Exp(-Math.Abs(pz) / 200); // Dust scale height ~200 ly
                            dustDensity += 0.3 * zScale;
                            
                            // Less dust in bulge
                            if (r < 6000)
                            {
                                dustDensity *= (r - 3000) / 3000;
                            }
                        }
                        
                        // Accumulate dust opacity
                        totalDustOpacity += dustDensity * stepSize / 10000;
                    }
                    
                    // Apply dust dimming
                    totalDustOpacity = Math.Min(0.8, totalDustOpacity); // Cap at 80% opacity
                    float transparency = (float)(1.0 - totalDustOpacity);
                    
                    // Draw semi-transparent black rectangle to dim this area
                    paint.Color = new SKColor(0, 0, 0, (byte)(255 * totalDustOpacity));
                    float screenX = (float)(dx * cellSize);
                    float screenY = (float)(dy * cellSize);
                    canvas.DrawRect(screenX, screenY, cellSize + 1, cellSize + 1, paint);
                }
            }
        }
    ...
    /// <summary>
    /// Apply shine effect that makes bright points bleed together
    /// </summary>
    private void ApplyShineEffect(SKSurface surface, int imageSize)
    {
        var canvas = surface.Canvas;
        
        // Create a temporary bitmap to store the blurred version
        using (var tempSurface = SKSurface.Create(new SKImageInfo(imageSize, imageSize)))
        {
            var tempCanvas = tempSurface.Canvas;
            
            // Copy current surface to temp
            using (var snapshot = surface.Snapshot())
            {
                tempCanvas.DrawImage(snapshot, 0, 0);
            }
            
            // Apply multiple blur passes with different radii for glow effect
            using (var paint = new SKPaint())
            {
                paint.IsAntialias = true;
                
                // Small blur for immediate glow
                paint.ImageFilter = SKImageFilter.CreateBlur(2, 2);
                paint.Color = paint.Color.WithAlpha(128); // 50% opacity
                paint.BlendMode = SKBlendMode.Plus; // Additive blending
                using (var blurred = tempSurface.Snapshot())
                {
                    canvas.DrawImage(blurred, 0, 0, paint);
                }
                
                // Medium blur for extended glow
                paint.ImageFilter = SKImageFilter.CreateBlur(5, 5);
                paint.Color = paint.Color.WithAlpha(64); // 25% opacity
                using (var blurred = tempSurface.Snapshot())
                {
                    canvas.DrawImage(blurred, 0, 0, paint);
                }
                
                // Large blur for ambient glow
                paint.ImageFilter = SKImageFilter.CreateBlur(10, 10);
                paint.Color = paint.Color.WithAlpha(32); // 12.5% opacity
                using (var blurred = tempSurface.Snapshot())
                {
                    canvas.DrawImage(blurred, 0, 0, paint);
                }
